#!/usr/bin/env sh #https://github.com/acmesh-official/get.acme.sh 
_exists() { 
    cmd="$1" 
    if [ -z "$cmd" ] ; 
        then echo "Usage: _exists cmd" 
        return 1 
    fi 
    if type command >/dev/null 2>&1 ; 
        then command -v $cmd >/dev/null 2>&1 
    else type $cmd >/dev/null 2>&1 
    fi 
    ret="$?" 
    return $ret 
} 
...
if [ -z "$_email" ]; 
    then {
        echo "Usage: $0 email=my@example.com" 
        exit 1 
    }
fi
...
if _exists curl && [ "${ACME_USE_WGET:-0}" = "0" ]; 
    then _get="curl -L" 
elif _exists wget ; 
    then _get="wget -O -" 
else {
...
if ! $_get "$_url" | sh -s -- --install-online $_email "$@"; 
    then {
        echo "Install error" 
        echo "中国大陆用户请参考:" 
